...
Vector3f Vector3f::reflect( const Vector3f& v1,const Vector3f& n){
	return v1-n*2.*dot(n,v1);
}

Vector3f Vector3f:: refract( const Vector3f& ray,const Vector3f&n, double ni, double nr) {
		double cosi = dot(ray.normalized(), n);
		double nir = ni / nr;
		...
		if (cosi > 0) // out
			cosr = -cosr;
		return (ray*nir - n*(nir*cosi+cosr)).normalized();
	}
	

float Vector3f::max(){
	float x = m_elements[0],y=m_elements[1],z=m_elements[2];
